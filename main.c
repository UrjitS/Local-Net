...
static void cmd_verify_oob(void) {
    ...
    for (int i = 0; i < OOB_COMMITMENT_SIZE; i++) {
        const char byte_str[3] = { code_input[i * 2], code_input[i * 2 + 1], '\0' };
        char *end_ptr;
...
static void cmd_set_static_oob(void) {
    printf("Enter even length static OOB token (hex string, e.g., A1B2C3D4): ");
    fflush(stdout);
        ...
        switch (hdr.message_type) {
            ...
            case MSG_DATA: {
                ...
                if (len >= 16 && parse_network(data + 8, len - 8, &net) == 0) {
                    ...
                    // Check if frame has a security block appended
                    if (len >= expected_encrypted) {
                        ...
                        const int dec_result = decrypt_frame(&g_session_mgr, net.source_id, header_bytes, sizeof(header_bytes), network_bytes, sizeof(network_bytes), raw_payload, payload_len,
                                                             &sec_block,
            ...
            case MSG_KEY_EXCHANGE: {
                ...
                if (handle_key_exchange(&g_session_mgr, sender_id, &kex_msg, &response, &need_response) != 0) {
                ...
                if (need_response && g_ble_manager) {
                    ...
                    const size_t resp_len = serialize_key_exchange_ext(&response, resp_payload,sizeof(resp_payload));
                    if (resp_len > 0) {
                        ...
                        const size_t total = serialize_packet(&resp_pkt, resp_buffer, sizeof(resp_buffer));
                        if (total > 0) {
                ...
                log_info(TAG, "Key exchange with 0x%08X: session in %s state", sender_id, session_state_to_string(session_find_by_peer(&g_session_mgr, sender_id) ? session_find_by_peer(&g_session_mgr, sender_id)->state : SESSION_STATE_EMPTY));
                break;
            }
            default:
