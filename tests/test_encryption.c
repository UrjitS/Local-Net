...
static void test_ecdh_generate_keypair(void) {
    ...
    const uint8_t zeros[X25519_KEY_SIZE] = {0};
    assert(memcmp(pub, zeros, X25519_KEY_SIZE) != 0);
    assert(memcmp(private_key, zeros, X25519_KEY_SIZE) != 0);

    // Two keypair should be different
...
static void test_ecdh_shared_secret(void) {
    ...
    assert(ecdh_generate_keypair(pub_a, private_key_a) == 0);
    assert(ecdh_generate_keypair(pub_b, private_key_b) == 0);

    // Both sides compute the same shared secret
    assert(ecdh_compute_shared_secret(secret_a, private_key_a, pub_b) == 0);
    assert(ecdh_compute_shared_secret(secret_b, private_key_b, pub_a) == 0);
...
static void test_aes128_ctr(void) {
    ...
    const char * plaintext = "This is a test of AES-128-CTR encryption.";
...
static void test_key_exchange_handshake(void) {
    ...
    const struct encryption_session * sess_a = session_find_by_peer(&mgr_a, node_b_id);
    assert(sess_a != NULL);
    ...
    const struct encryption_session * sess_b = session_find_by_peer(&mgr_b, node_a_id);
    assert(sess_b != NULL);
    assert(sess_b->state == SESSION_STATE_OOB_PENDING);

    // Step 3: Node A receives response 
    int need_response_2 = 0;
    assert(handle_key_exchange(&mgr_a, node_b_id, &kex_response, NULL, &need_response_2) == 0);
...
static void test_oob_mismatch(void) {
    ...
    const uint8_t wrong_code[OOB_COMMITMENT_SIZE] = {0xFF, 0xFF, 0xFF, 0xFF};
    const int result = verify_oob_code(&mgr_a, 0x2222, wrong_code);
...
static void test_frame_encrypt_decrypt(void) {
    ...
    const uint8_t header_data[8] = {0x10, 0x00, 0x01, 0x0F, 0x00, 0x13, 0x00, 0x01};
    ...
    const int dec_result = decrypt_frame(&mgr_b, node_a_id, header_data, 8, network_data, 8,
...
static void test_kex_serialization(void) {
    ...
    uint8_t buffer[64];
    const size_t written = serialize_key_exchange_ext(&kex, buffer, sizeof(buffer));
...
static void test_multiple_frames(void) {
    ...
    const struct encryption_session * sa = session_find_by_peer(&mgr_a, node_b);
    const struct encryption_session * sb = session_find_by_peer(&mgr_b, node_a);
