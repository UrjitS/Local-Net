...
#define SESSION_MAX_LIFETIME_SECONDS  86400
 ...
 */
typedef void (* oob_display_callback_t)(uint32_t peer_id, const uint8_t short_code[OOB_COMMITMENT_SIZE], enum oob_method oob_method);
...
int session_manager_set_static_oob_token(struct session_manager * mgr, const uint8_t * token, size_t token_len);
 ...
 */
int initiate_key_exchange(struct session_manager * mgr, uint32_t peer_id, struct key_exchange_ext_message * kex_out);
 ...
 */
int handle_key_exchange(struct session_manager * mgr, uint32_t peer_id, const struct key_exchange_ext_message * kex_msg, struct key_exchange_ext_message * response_out, int * need_response);
 ...
 */
int verify_oob_code(struct session_manager * mgr, uint32_t peer_id, const uint8_t user_code[OOB_COMMITMENT_SIZE]);

/**
 * Get the OOB short code for a session (for display purposes).
 * @return 0 on success, -1 if no OOB_PENDING session exists
 */
int get_oob_code(struct session_manager * mgr, uint32_t peer_id, uint8_t code_out[OOB_COMMITMENT_SIZE]);
...
struct encryption_session *session_find_by_peer(struct session_manager * mgr, uint32_t peer_id);

struct encryption_session *session_find_by_key_id(struct session_manager * mgr, uint8_t key_id);

void session_destroy(struct session_manager * mgr, uint32_t peer_id);

int session_needs_rotation(const struct encryption_session * session, uint32_t current_time);

size_t session_check_oob_timeouts(struct session_manager * mgr, uint32_t current_time);

size_t session_check_lifetimes(struct session_manager * mgr, uint32_t current_time);
...
size_t serialize_key_exchange_ext(const struct key_exchange_ext_message * kex, uint8_t * buffer, size_t buffer_size);

int parse_key_exchange_ext(const uint8_t * buffer, size_t buffer_size, struct key_exchange_ext_message * kex);
...
int hkdf_sha256(const uint8_t * input_key_material, size_t ikm_len, const uint8_t * info, size_t info_len, uint8_t * output_key_material, size_t okm_len);
