...
int ecdh_generate_keypair(uint8_t public_key[X25519_KEY_SIZE], uint8_t private_key[X25519_KEY_SIZE]) {
...
int ecdh_compute_shared_secret(uint8_t shared_secret[X25519_SHARED_SECRET_SIZE], const uint8_t private_key[X25519_KEY_SIZE], const uint8_t remote_public_key[X25519_KEY_SIZE]) {
...
int hkdf_sha256(const uint8_t * input_key_material, const size_t ikm_len, const uint8_t * info, const size_t info_len, uint8_t * output_key_material, const size_t okm_len) {
    if (!input_key_material || !output_key_material || okm_len == 0) return -1;

    // HKDF-Extract: PRK = HMAC-SHA256(salt="", IKM) 
    uint8_t prk[HMAC_SHA256_SIZE];
    const uint8_t salt[HMAC_SHA256_SIZE] = {0};
    ...
    while (offset < okm_len) {
        ...
        const size_t to_copy = (okm_len - offset < HMAC_SHA256_SIZE) ? (okm_len - offset) : HMAC_SHA256_SIZE;
...
int aes128_ctr_crypt(const uint8_t key[AES128_KEY_SIZE], const uint8_t iv[AES128_IV_SIZE], const uint8_t * input, const size_t input_len, uint8_t * output) {
...
int hmac_sha256(const uint8_t * key, const size_t key_len, const uint8_t * data, const size_t data_len, uint8_t output[HMAC_SHA256_SIZE]) {
...
int hmac_sha256_truncated(const uint8_t * key, const size_t key_len, const uint8_t * data, const size_t data_len, uint8_t output[HMAC_SHA256_TRUNCATED_SIZE]) {
...
void crypto_random_bytes(uint8_t * buffer, const size_t len) {
    if (buffer && len > 0) {
        randombytes_buf(buffer, len);
    }
}

void crypto_secure_wipe(void * buffer, const size_t len) {
    if (buffer && len > 0) {
 ...
 */
static int derive_session_keys(struct encryption_session * session, const uint8_t shared_secret[X25519_SHARED_SECRET_SIZE], const uint32_t src_id, const uint32_t dst_id, const uint8_t * static_oob_token, const size_t static_oob_token_len) {
    ...
    // NOLINTNEXTLINE
    if (hkdf_sha256(ikm, ikm_len, (const uint8_t *)master_info, strlen(master_info), session->master_key, MASTER_KEY_SIZE) != 0) {
    ...
    // NOLINTNEXTLINE
    if (hkdf_sha256(session->master_key, MASTER_KEY_SIZE, ek_info, ek_info_len, session->encryption_key, ENCRYPTION_KEY_SIZE) != 0) {
        return -1;
    }
    ...
    // NOLINTNEXTLINE
    if (hkdf_sha256(session->master_key, MASTER_KEY_SIZE, ak_info, ak_info_len, session->auth_key, AUTH_KEY_SIZE) != 0) {
        return -1;
    }
 ...
 */
static int compute_oob_commitment(struct encryption_session * session, const uint8_t a_public[X25519_KEY_SIZE], const uint8_t b_public[X25519_KEY_SIZE], const uint32_t src_id, const uint32_t dst_id) {
    ...
    // NOLINTNEXTLINE
    if (hmac_sha256(session->auth_key, AUTH_KEY_SIZE, data, offset, full_hmac) != 0) {
        return -1;
    }
 ...
 */
static enum oob_method negotiate_oob_method(const uint8_t local_supported, const uint8_t remote_supported) {
 ...
 */
static int complete_handshake(const struct session_manager * mgr, struct encryption_session * session, const uint32_t src_id, const uint32_t dst_id) {
    uint8_t shared_secret[X25519_SHARED_SECRET_SIZE];

    // Compute shared secret: X25519(local_private, remote_public) 
    if (ecdh_compute_shared_secret(shared_secret, session->local_private_key, session->remote_public_key) != 0) {
    ...
    // NOLINTNEXTLINE
    if (derive_session_keys(session, shared_secret, src_id, dst_id, static_token, token_len) != 0) {
        sodium_memzero(shared_secret, sizeof(shared_secret));
        return -1;
    }
    ...
    // Compute OOB commitment 
    // NOLINTNEXTLINE
    if (compute_oob_commitment(session, a_public, b_public, src_id, dst_id) != 0) {
        return -1;
    }
    ...
    if (mgr->oob_display_cb) {
        mgr->oob_display_cb(session->peer_id, session->oob_commitment, session->negotiated_oob_method);
    }

    return 0;
}

int session_manager_init(struct session_manager * mgr, const uint32_t local_device_id) {
    if (!mgr) return -1;
...
// NOLINTNEXTLINE
void session_manager_set_oob_callback(struct session_manager * mgr, oob_display_callback_t callback) {
    if (mgr) {
        mgr->oob_display_cb = callback;
    }
}

int session_manager_set_static_oob_token(struct session_manager * mgr, const uint8_t * token, const size_t token_len) {
...
struct encryption_session * session_find_by_peer(struct session_manager * mgr, const uint32_t peer_id) {
    if (!mgr) return NULL;
    for (size_t i = 0; i < MAX_SESSIONS; i++) {
        if (mgr->sessions[i].state != SESSION_STATE_EMPTY &&
...
struct encryption_session * session_find_by_key_id(struct session_manager * mgr, const uint8_t key_id) {
    if (!mgr) return NULL;
    for (size_t i = 0; i < MAX_SESSIONS; i++) {
        if (mgr->sessions[i].state != SESSION_STATE_EMPTY &&
...
void session_destroy(struct session_manager * mgr, const uint32_t peer_id) {
    if (!mgr) return;
    for (size_t i = 0; i < MAX_SESSIONS; i++) {
        if (mgr->sessions[i].state != SESSION_STATE_EMPTY &&
            mgr->sessions[i].peer_id == peer_id) {
...
int session_needs_rotation(const struct encryption_session * session, const uint32_t current_time) {
...
size_t session_check_oob_timeouts(struct session_manager * mgr, const uint32_t current_time) {
    if (!mgr) return 0;
    size_t count = 0;
...
size_t session_check_lifetimes(struct session_manager * mgr, const uint32_t current_time) {
    if (!mgr) return 0;
    size_t count = 0;
...
const char *session_state_to_string(const enum session_state state) {
    switch (state) {
...
const char *oob_method_to_string(const enum oob_method method) {
    switch (method) {
...
int initiate_key_exchange(struct session_manager * mgr, const uint32_t peer_id, struct key_exchange_ext_message * kex_out) {
    if (!mgr || !kex_out || peer_id == 0) return -1;

    // Check if session already exists 
    const struct encryption_session * existing = session_find_by_peer(mgr, peer_id);
    if (existing) {
...
int handle_key_exchange(struct session_manager * mgr, const uint32_t peer_id, const struct key_exchange_ext_message * kex_msg, struct key_exchange_ext_message * response_out, int * need_response) {
    ...
    * need_response = 0;
    ...
    if (kex_msg->kex_type == KEX_TYPE_REQUEST) {
        ...
        const uint32_t src_id = peer_id;
        const uint32_t dst_id = mgr->local_device_id;
        ...
        const uint32_t src_id = mgr->local_device_id;
        const uint32_t dst_id = peer_id;
...
int verify_oob_code(struct session_manager * mgr, const uint32_t peer_id, const uint8_t user_code[OOB_COMMITMENT_SIZE]) {
...
int get_oob_code(struct session_manager * mgr, const uint32_t peer_id, uint8_t code_out[OOB_COMMITMENT_SIZE]) {
 ...
 */
static void build_iv(const uint8_t nonce[NONCE_SIZE], const uint32_t frame_counter, uint8_t iv[AES128_IV_SIZE]) {
 ...
 */
static int compute_mac(const uint8_t * auth_key, const uint8_t * header_data, const size_t header_len, const uint8_t * network_data, const size_t network_len, const uint8_t * encrypted_payload, const size_t encrypted_len, const uint32_t frame_counter, uint8_t mac_out[HMAC_SHA256_TRUNCATED_SIZE]) {
    ...
    const int ret = hmac_sha256_truncated(auth_key, AUTH_KEY_SIZE, mac_input, total_len, mac_out);
    free(mac_input);
    return ret;
}

int encrypt_frame(struct session_manager * mgr, const uint32_t destination_id, const uint8_t * header_data, const size_t header_len, const uint8_t * network_data, const size_t network_len, const uint8_t * plaintext, const size_t plaintext_len, uint8_t ** ciphertext_out, size_t * ciphertext_len_out, struct security_block * security_out) {
    ...
    // Encrypt 
    if (aes128_ctr_crypt(session->encryption_key, iv, plaintext, plaintext_len, ciphertext) != 0) {
        free(ciphertext);
    ...
    if (compute_mac(session->auth_key, header_data, header_len, network_data, network_len, ciphertext, plaintext_len, session->send_frame_counter, mac) != 0) {
        free(ciphertext);
...
int decrypt_frame(struct session_manager * mgr, const uint32_t source_id, const uint8_t * header_data, const size_t header_len, const uint8_t * network_data, const size_t network_len, const uint8_t * ciphertext, const size_t ciphertext_len, const struct security_block * security, uint8_t ** plaintext_out, size_t * plaintext_len_out) {
    ...
    // Decrypt 
    if (aes128_ctr_crypt(session->encryption_key, iv, ciphertext, ciphertext_len, plaintext) != 0) {
        free(plaintext);
...
size_t serialize_key_exchange_ext(const struct key_exchange_ext_message * kex, uint8_t * buffer, const size_t buffer_size) {
...
int parse_key_exchange_ext(const uint8_t * buffer, const size_t buffer_size, struct key_exchange_ext_message * kex) {
