...
int process_route_reply(struct mesh_node *node, uint32_t request_id, const uint8_t route_cost, const uint32_t *forward_path, const uint8_t forward_path_len) {
    ...
    const uint32_t next_hop = forward_path[our_position + 1];
    const uint32_t destination_id = forward_path[forward_path_len - 1];

    // Calculate hop count from us to destination
    const uint8_t hop_count = forward_path_len - our_position - 1;

    // Calculate route cost
    const float cost = (float)route_cost;
...
int forward_packet(struct mesh_node *node, const uint32_t destination_id, uint8_t *ttl, uint32_t *next_hop_out) {
...
int create_route_request(struct mesh_node *node, const uint32_t destination_id, struct route_request *req_out) {
...
int handle_route_request(struct mesh_node *node, const struct route_request *req, const uint32_t sender_id, struct route_request_result *result) {
    ...
    if (node->node_type != EDGE_NODE) {
        ...
        if (cached_route && cached_route->is_valid) {
            ...
            if (req->reverse_path && req->reverse_path_len > 0) {
                memcpy(result->updated_reverse_path, req->reverse_path, req->reverse_path_len * sizeof(uint32_t));
            }
            result->updated_reverse_path[result->updated_path_len - 1] = node->device_id;

            // Add route back to originator 
            if (originator_id != 0 && originator_id != node->device_id) {
                add_route(node->routing_table, originator_id, sender_id, new_hop_count, (float)new_hop_count, get_current_timestamp());
            }
...
int create_route_reply(struct mesh_node *node, const uint32_t request_id, const uint32_t *reverse_path, const uint8_t reverse_path_len, struct route_reply *reply_out) {
...
int handle_route_reply(struct mesh_node *node, const struct route_reply *reply, const uint32_t sender_id, struct route_reply_result *result) {
...
int add_pending_route_request(struct mesh_node *node, const uint32_t request_id, const uint32_t destination_id) {
    if (!node) return -1;
...
size_t check_route_request_timeouts(struct mesh_node *node, const uint32_t current_time, uint32_t *timed_out_destinations, const size_t max_count) {
...
size_t get_connected_neighbors(struct mesh_node *node, uint32_t *neighbors, const size_t max_count, const uint32_t exclude_id) {
...
uint16_t queue_packet_for_transmission(struct pending_packet_queue *queue,
    ...
    if (queue->next_sequence_number == 0) queue->next_sequence_number = 1;
...
size_t handle_route_discovery_complete(struct pending_packet_queue *queue, const uint32_t destination_id) {
    if (!queue) return 0;

    size_t count = 0;
    const uint32_t current_time = get_current_timestamp() * 1000;
    ...
    for (size_t i = 0; i < MAX_PENDING_PACKETS; i++) {
        ...
        if (packet->state == PACKET_STATE_AWAITING_ROUTE &&
            ...
            packet->next_retry_timestamp = current_time;
...
int handle_route_discovery_failed(struct pending_packet_queue *queue, const uint32_t destination_id) {
    if (!queue) return -1;
...
size_t check_retransmission_timeouts(struct pending_packet_queue *queue, const uint32_t current_time_ms, uint16_t *retry_sequence_numbers, const size_t max_count) {
...
struct pending_packet *get_pending_packet(struct pending_packet_queue *queue, const uint16_t sequence_number) {
...
void update_retry_timing(struct pending_packet *packet, const uint32_t current_time_ms) {
    ...
    // Exponential backoff with cap TCP
...
int make_forwarding_decision(struct mesh_node *node, const uint32_t destination_id, uint8_t *ttl, struct forwarding_decision *decision) {
    ...
    const struct routing_entry * route = find_best_route(node->routing_table, destination_id);
    if (route && route->is_valid) {
        decision->action = 0;
    ...
    decision->action = -2;
    decision->error_code = 0x01;

    return 0;
}

int update_route_cost_on_ack(struct routing_table *table, struct connection_table *conn_table, const uint32_t destination_id, const uint32_t next_hop, const int success) {
    if (!table) return -1;
    ...
    if (conn_table) {
        ...
        const struct connection_entry * conn = find_connection(conn_table, next_hop);
        if (conn && conn->link_quality > 0.0f) {
